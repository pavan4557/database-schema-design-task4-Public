...
-- 3. AVG: Average published year of books
SELECT AVG(published_year) AS avg_published_year FROM Book;

-- 4. SUM: (example, sum of numeric column, assuming Loan has fines)
-- ALTER TABLE Loan ADD COLUMN fine DECIMAL(10,2) DEFAULT 0; -- optional
-- SELECT SUM(fine) AS total_fines FROM Loan;

-- 5. MAX / MIN: Most recent and oldest book
SELECT MAX(published_year) AS newest_book, MIN(published_year) AS oldest_book FROM Book;

-- 6. GROUP BY with HAVING: Categories with more than 1 book
SELECT category_id, COUNT(*) AS book_count
FROM Book
GROUP BY category_id
HAVING COUNT(*) > 1;

-- 7. Members who have borrowed more than 0 books (using JOIN)
...
-- 8. COUNT DISTINCT: Count unique categories
SELECT COUNT(DISTINCT category_id) AS unique_categories FROM Book;

-- 9. ROUND() example: Average published year rounded
